...
setInterval( function () {
   ...
   req1.write(JSON.stringify({x: x, y: liquidTemp(x)})+ "\n")
   req2.write(JSON.stringify({x: x, y: airTemp(x)})+ "\n")
}, 50)



function liquidTemp(x) {
   
  liquidTemp.read_value( function(datastr){
...
function airTemp(x) {
  ...
  humi2c.read_value( function(datastr2){
